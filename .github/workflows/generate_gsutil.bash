...
export COMP_LINE="gsutil"
export COMP_POINT=0
export COMP_TYPE

COMMANDS="$(gsutil 8>&1 9>&2 1>/dev/null 2>&1)"

ROOT="$(git rev-parse --show-toplevel)"
FILE="$ROOT/completions/gsutil.fish"
...
{
  COMMANDS="$(gsutil 8>&1 9>&2 1>/dev/null 2>&1 | xargs)"

  if [ "$COMMANDS" != "" ] && [[ "$COMMANDS" != *ERROR:* ]]; then
    if [ "$COMP_LINE" == "gsutil " ]; then
      echo "complete -c gsutil -x -n __fish_use_subcommand -a \"$COMMANDS\"" >> "$FILE"
    else
      local TARGET="gsutil "
      local REPLACE=""
      ...
      echo "complete -c gsutil -x -n \"__fish_equals_subcommands $USING_COMMAND\" -a \"$COMMANDS\"" >> "$FILE"
    fi
